...
orderSchema.index(
  { orderNumber: 1 },
  {
    ...
    partialFilterExpression: {
      orderNumber: { $exists: true, $type: "string" },
    },
  }
);
...
orderSchema.index({ "ordereditems.product": 1 });
orderSchema.index({ "ordereditems.status": 1 });
orderSchema.index({ paymentMethod: 1, status: 1 });
orderSchema.index({ createdOn: -1 }); // For sorting by latest
orderSchema.index({ userId: 1, status: 1 }); // For user-specific status filters
orderSchema.index({ deliveredAt: -1 }); // For delivery reports
orderSchema.index({ totalRefunded: 1 }); // For refund queries
orderSchema.index({ "ordereditems.returnStatus": 1 }); // For return queries
...
orderSchema.pre("save", function (next) {
  ...
  if (this.isNew) {
    ...
    if (!this.currentFinalAmount) {
      this.currentFinalAmount =
        this.originalFinalAmount || this.finalAmount || 0;
    }
  }
