    ...
    try {
      ...
      const transactions = await Transaction.find({
        $or: [
          ...
          { userId: { $in: await User.find({ name: new RegExp(search, "i") }).distinct('_id') } } // Search by user ID
        ]
      })
      ...
      const totalTransactions = await Transaction.countDocuments({
        $or: [
          ...
          { userId: { $in: await User.find({ name: new RegExp(search, "i") }).distinct('_id') } }
        ]
      });
  ...
  try {
    ...
    const docDefinition = {
      content: [
        ...
        {
          text: `Filter: ${filterType.charAt(0).toUpperCase() + filterType.slice(1)} ${
